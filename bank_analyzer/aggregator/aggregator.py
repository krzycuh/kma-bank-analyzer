...
class Aggregator:
    ...
    def aggregate(self, transactions: List[Transaction]) -> Dict[str, Any]:
        ...
        for trans in transactions:
            ...
            # Add to category (expense and income tracked separately,
            # so reports can reconcile sums per direction)
            cat_data = data[year][month]['categories'][cat_main][cat_sub]
            cat_data['total'] += trans.amount
            if trans.transaction_type == 'expense':
                cat_data['expense'] += trans.amount
            else:
                cat_data['income'] += trans.amount
            cat_data['count'] += 1
            ...
            for month in sorted(data[year].keys()):
                ...
                for cat_main, subcats in month_data['categories'].items():
                    ...
                    for cat_sub, values in subcats.items():
                        ...
                        year_data['categories_year'][cat_main][cat_sub]['expense'] += values['expense']
                        year_data['categories_year'][cat_main][cat_sub]['income'] += values['income']
