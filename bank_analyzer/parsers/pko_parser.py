...
class PKOParser(BaseParser):
    ...
    def _extract_counterparty(self, desc_columns: List[str]) -> str:
        ...
        for col in desc_columns:
            if not col:
                continue
            # Non-greedy up to "Miasto:"/"Kraj:" or end of column.
            # (A negated [^M] class would break here: with IGNORECASE it
            # rejects every address containing "m", e.g. www.* domains.)
            match = re.search(
                r'Adres:\s*(.+?)(?=\s*Miasto:|\s*Kraj:|$)',
                col,
                re.IGNORECASE
            )
            if match:
        ...
        for col in desc_columns:
            ...
            if text:
                ...
                if parts and parts[0]:
                    ...
                    # Reject useless candidates: payment reference numbers
                    # (digits only) and bare labels like "Tytuł:"
                    if name and len(name) > 2:
