...
class RuleEngine:
    ...
    def _match_exclude_rule(
        ...
        if rule_type and transaction.transaction_type != rule_type:
            return False
    ...
    def categorize(self, transaction: Transaction) -> Tuple[str, str]:
        ...
        """
        # Check cache (transaction_type included - rules can be type-scoped)
        cache_key = (
            f"{transaction.counterparty.lower()}|"
    ...
    def _match_rule(
        ...
        # Optional type scope: rule applies only to 'expense' or 'income'
        rule_type = rule.get('transaction_type')
        if rule_type and transaction.transaction_type != rule_type:
            return False
